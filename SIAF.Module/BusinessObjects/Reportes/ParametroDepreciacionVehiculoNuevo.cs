...
namespace SIAF.Module.BusinessObjects.Reportes
{
    ...
    public class ParametroDepreciacionVehiculoNuevo : ReportParametersObjectBase
    {
        public ParametroDepreciacionVehiculoNuevo(IObjectSpaceCreator provider) : base(provider) { }
        ...
        private void GenerateData()
        {
            ...
            ICollection<Vehiculo> ListadoEquipos = ObjectSpace.GetObjects<Vehiculo>(criteriaFinal);


            if (!ReferenceEquals(ListadoEquipos, null))
            {
                foreach (Vehiculo ObjEquipos in ListadoEquipos)
                {
                    ...
                    if (validar == true)
                    {
                        ...
                        Objreporte.Marca = ObjEquipos.Marca.Descripcion;
                        Objreporte.Modelo = ObjEquipos.Modelo;
                        Objreporte.Placa = ObjEquipos.Placa;
                        Objreporte.NoMotor = ObjEquipos.Motor;
                        Objreporte.NoChasis = ObjEquipos.NumeroChasis;
                        Objreporte.Axo = Convert.ToString(ObjEquipos.AnioFabricacion);
